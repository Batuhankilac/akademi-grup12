# 👥 Grup 12

## 👤 Ekip Üyeleri
- **Emine Kilik** – *Scrum Master & Product Owner & Developer*   
- **Azra Baltalı** – *Developer*  
- **Batuhan Kılaç** – *Developer*  
- **Ömer Faruk Çakıllı** – *Developer*

---

## 📅 Sprint 1

### 🗂️ Sprint Notları
- Proje konusu belirlendi ve temel oyun yapısına uygun olarak ilk sprint planlaması yapıldı.
- Görevler iş paketlerine ayrıldı, her bir görev sorumlu kişilere dağıtıldı.
- Multiplayer, envanter, nesne etkileşimi ve görev sistemlerinin iskeleti oluşturuldu.
- Asset araştırmalarına başlandı, örnek sahne ve ışıklandırma çalışmaları yapıldı.
- Takım içi iletişim için WhatsApp kullanılması ve haftada iki sesli toplantı yapılması kararlaştırıldı.
- Bireysel yoğunluklar nedeniyle bazı görevlerde gecikmeler yaşandı, bu durum gelecek sprint planlamasına yansıtıldı.
- Toplantı saatlerinin esnek tutulmasına ve ilerleme paylaşımlarının daha sık yapılmasına karar verildi.
- Karakter ve NPC sistemleri bir sonraki sprintte öncelikli olarak ele alınacak.


### 📈 Tahmin Edilen Tamamlanacak Puan
- Planlanan Puan: 100
- Tamamlanan Puan: 79

### 🔍 Tahmin Mantığı
- Planlama aşamasında görev listesi çıkarılarak her bir iş için gereken araştırma ve geliştirme süreleri tahmin edildi.
- Toplamda 300 puanlık iş belirlendi, bu işler 3 sprinte bölünerek her sprint için yaklaşık 100 puan hedeflendi.
- İlk sprintte temel mekanikler; ikinci sprintte oyun akışı ve tamamlayıcı sistemler; üçüncü sprintte ise oyunu zenginleştirecek detaylar planlandı.

### 📞 Daily Scrum
- Günlük ilerleme paylaşımları **WhatsApp** üzerinden yapıldı.
- Haftada **iki kez sesli toplantı** gerçekleştirildi.
- [Daily & Toplantı Özetleri](https://drive.google.com/drive/folders/1WpfAkQ9AYgoq105I_wECK41KM1wiLsf_?usp=drive_link)

---

### 🧾 Sprint Board Güncellemeleri
<p align="center">
  <img src="sprint/board/Ekran görüntüsü 2025-07-06 234901.png" alt="Sprint Board 20.06.2025" width="600"/>
</p>

<p align="center">
  <img src="sprint/board/Ekran görüntüsü 2025-07-06 235123.png" alt="Sprint Board 06.07.2025" width="600"/>
</p>

---

### 🎮 Ürün Durumu
<p align="center">
  <img src="sprint/product/Ekran görüntüsü 2025-07-07 004047.png" width="600"/>
</p>

<p align="center">
  <img src="sprint/product/Ekran görüntüsü 2025-07-07 004227.png" width="600"/>
</p>

<p align="center">
  <img src="sprint/product/Ekran görüntüsü 2025-07-07 004927.png" width="600"/>
</p>

<p align="center">
  <img src="sprint/product/Ekran görüntüsü 2025-07-07 005005.png" width="600"/>
</p>

---
### Sprint Review
- Sprint sonunda multiplayer, envanter, nesne etkileşimi ve görev sistemleri başarıyla tamamlandı.
- Asset araştırmaları doğrultusunda örnek sahne ve ışıklandırma denemeleri yapıldı.
- Temel mekanikler büyük ölçüde tamamlandı, eksik kalan karakter ve NPC sistemleri bir sonraki sprintte geliştirilecek.

### Sprint Retrospective
- Sprint süresince bireysel yoğunluklar bazı görevlerde gecikmeye neden oldu.
- İletişim sıklığının arttırılması ve ilerleme güncellemelerinin daha düzenli paylaşılması gerektiği görüldü.
- Toplantıların sabit değil, esnek saatlerle planlanmasının takıma daha uygun olduğu gözlemlendi.

## 📅 Sprint 2

### 🗂️ Sprint Notları
- Projenin ana mekanikleri oluşturuldu.
- Npc takip ve yakalama sistemleri, karakter hareketi, kasa sistemi oluşturuldu.
- Sahne tasarımı genişletildi, oyun içi assetler ve sesler bulundu.
- Bireysel yoğunluklar nedeniyle birçok görev yapılmadı.
- Takımdan bir kişi aktif olmaması ve iletişime geçmemesi sebebiyle çıkarıldı.

### 📈 Tahmin Edilen Tamamlanacak Puan
- Planlanan Puan: 139
- Tamamlanan Puan: 60

### 🔍 Tahmin Mantığı
- Toplamda 300 puanlık iş belirlendi, bu işler 3 sprinte bölünerek her sprint için yaklaşık 100 puan hedeflendi.
- Önceki sprintte tamamlanamayan görevlerin bu sprintte tamamlanması hedeflenerek toplam puana eklendi.
- İlk sprintte temel mekanikler; ikinci sprintte oyun akışı ve tamamlayıcı sistemler; üçüncü sprintte ise oyunu zenginleştirecek detaylar planlandı.

### 📞 Daily Scrum
- Günlük ilerleme paylaşımları **WhatsApp** üzerinden yapıldı.
- Haftada **iki kez sesli toplantı** gerçekleştirildi.
- [Daily & Toplantı Özetleri](https://drive.google.com/drive/folders/1eA_eZdznPAODCo20t2vwwsXacvPHAIMW?usp=sharing)

---

### 🧾 Sprint Board Güncellemeleri
<p align="center">
  <img src="sprint2/board/Ekran görüntüsü 2025-07-21 005246.png" alt="Sprint Board 06.07.2025" width="600"/>
</p>

<p align="center">
  <img src="sprint2/board/Ekran görüntüsü 2025-07-21 005101.png" alt="Sprint Board 20.07.2025" width="600"/>
</p>

---

### 🎮 Ürün Durumu
<p align="center">
  <img src="sprint2/product/Ekran görüntüsü 2025-07-21 011312.png" width="600"/>
</p>

<p align="center">
  <img src="sprint2/product/Ekran görüntüsü 2025-07-21 011158.png" width="600"/>
</p>


---
### Sprint Review
- Sprint sonunda Npc takip ve yakalama sistemleri, karakter hareketi, kasa sistemi oluşturuldu.
- Sahne tasarımı genişletildi, oyun içi assetler ve sesler bulundu.
- Geliştirme süreci çok yavaşladı, istenilen hedefe ulaşılamadı.

### Sprint Retrospective
- Sprint süresince bireysel yoğunluklar projenin aksamasına neden oldu.
- Takımda aktif olmayan ve henüz hiç görev yapmamış üyeler bulunduğundan projenin basitleştirilmesi gerektiği gözlemlendi.

